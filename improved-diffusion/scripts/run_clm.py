...
class myDataset_tencent(object):
    ...
    def __getitem__(self, id):
        ...
        idx = 1 # 0为start token
        traj_data = self.split_data[id]['traj'] #mobile label
        consumption,gender,edu,age,job = self.split_data[id]['profile'] 
        ...
        elif self.control == 'consumption':
            consumption_label = consumption + 32391
            traj = np.append(traj,int(consumption_label))
...
def main():
    ...
    if dataset_name == 'mobile':
        ...
        path_learned = '{}/ema_0.9999_550000.pt'.format(filename) ## tencent
